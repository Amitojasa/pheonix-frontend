...
function DiceComponentOffline({ showTask, isOffline = false, diceMove, setDisableDice, disableDice, setDiceMove, playMove, changePlayerId, roomName }) {
    ...
    return (
        ...
        <View style={{ flex: 1, alignSelf: 'center', backgroundColor: "#3DBE29", padding: "3%", borderRadius: 10, bottom: -30 }}>
            <TouchableOpacity disabled={disableDice || showTask} onPress={() => {

                startRotateImage()
            }
                // (a || disableDice || isOffline) && (!showTask) ? false : myPlayerId != activePlayerId
            }>
                <Animated.Image
                    style={{
                        alignSelf: "center",
...
export default DiceComponentOffline